  ...
  return (
    <>
      ...
      <div className="hidden sm:block fixed top-0 left-0 right-0 z-50">
        <div className="w-[75vw] mx-auto px-[3vw] py-[4.4vh] rounded-b-full bg-white/70 backdrop-blur-md shadow-lg flex items-center justify-between relative transition-all duration-500">
          {/* Cafe Sign */}
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 flex gap-[5.5vw]">
            {navItems.map(({ label, icon }) => (
              <div
                key={label}
                className="h-[5vh] px-[2.5vw] py-[1vh] bg-white text-gray-800 rounded-full shadow text-[1vw] font-poetsen hover:bg-neutral-200 transition-all cursor-pointer flex items-center relative"
              >
                <img
                  src={icon}
                  alt={`${label} icon`}
